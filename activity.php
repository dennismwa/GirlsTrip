...
$activity_query = "SELECT * FROM activities WHERE slug = '$slug' AND status = 'active'";
...
$tours_query = "SELECT t.*, l.name as location_name,
                ...
                WHERE ta.activity_id = " . $activity['id'] . " AND t.status = 'published'
                ORDER BY t.is_featured DESC, t.created_at DESC";
$tours = $conn->query($tours_query);

$page_title = $activity['name'] . ' - Activities - Girls Trip';
$meta_description = $activity['description'] ?? 'Explore ' . $activity['name'] . ' tours and experiences';
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center gap-4 mb-4">
            <?php if ($activity['icon']): ?>
                <i class="<?php echo $activity['icon']; ?> text-5xl text-white"></i>
            <?php endif; ?>
            <div>
                ...
                <?php if ($activity['description']): ?>
                    <p class="text-pink-100"><?php echo htmlspecialchars($activity['description']); ?></p>
                <?php endif; ?>
            </div>
        </div>
    </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <?php else: ?>
            <div class="text-center py-16 bg-white rounded-lg shadow-md">
                ...
                <h3 class="text-2xl font-bold text-gray-900 mb-2">No Tours for This Activity</h3>
                <p class="text-gray-600 mb-6">Check back soon or explore other activities</p>
